# servidor_egua_gemini_rag.py
# Requisitos: pip install flask flask-cors google-generativeai faiss-cpu numpy
...
GENERATION_MODEL = "gemini-2.0-flash"  # Modelo de geração mais recente e eficiente

generation_llm = genai.GenerativeModel(GENERATION_MODEL)

# -------------------------
# Configurações do RAG
# -------------------------
INDEX_DIR = "../../faiss_indexes/gemini_constructor"
JSONL_PATH = "../../rag/RAG_exemplos_hibrido.jsonl"
DOCS_PATH = "../../rag/docs.txt"
EMB_DIM = 768 # Dimensão do embedding-001
...
def gerar_embedding(texto: str) -> np.ndarray:
    """Gera o embedding de um texto usando a API do Gemini."""
    try:
        # Garante que o texto não seja vazio
        if not texto.strip():
...
def construir_ou_carregar_indice():
    ...
    index = faiss.IndexFlatIP(EMB_DIM) # Inner Product para vetores normalizados
    metadados = []
...
# ---------------------------------------------------------------------------------
# O "CÉREBRO" DA APLICAÇÃO: PROMPT MESTRE MODIFICADO PARA USAR RAG
# ---------------------------------------------------------------------------------
SYSTEM_PROMPT = """
...
# --------------------------------
def gerar_codigo_hibrido_com_gemini(json_data: dict, k: int = 5) -> str:
    ...
    # 3. ETAPA DE GERAÇÃO
    try:
        response = generation_llm.generate_content(
            prompt_completo,
            generation_config={"temperature": 0.0}
        )
        codigo_limpo = response.text.strip().replace("`", "").replace("égua", "")
        return codigo_limpo
    except Exception as e:
...
def gerar_codigo_endpoint():
    try:
        ...
        codigo_gerado = gerar_codigo_hibrido_com_gemini(json_data)
...
if __name__ == "__main__":
    # É uma boa prática usar uma porta diferente para cada servidor
